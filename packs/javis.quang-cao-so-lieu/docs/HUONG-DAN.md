...
| Trợ lý | Skill |
|---|---|
| `qc-kiem-chung` | `grounded-citations` (cần `javis.hermes-research`), `verification-before-completion` (cần `javis.superpowers`) |
| `qc-phan-tich-quang-cao` | `grounded-citations` (cần `javis.hermes-research`), `query-wiki` (hệ thống), `notes` (hệ thống) |
| `qc-phan-tich-so-lieu` | `xlsx` (cần `javis.hermes-office`), `notes` (hệ thống) |

Skill ghi **(hệ thống)** có sẵn trong mọi brain, không phải cài gì thêm.

Skill còn lại đến từ các gói kỹ năng trong kho: `javis.hermes-office`, `javis.hermes-research`, `javis.superpowers`. Javis không có cơ chế khai phụ thuộc giữa các gói, nên nếu bạn chưa cài chúng thì agent vẫn chạy bình thường: mỗi agent được dạy là gọi skill không có thì đi tiếp bằng năng lực sẵn có rồi báo lại một dòng cho bạn biết thiếu gì.
